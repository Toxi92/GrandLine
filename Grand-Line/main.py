...
def render_profile(username):
    if username==Admin.name:
        return  render_template('your_profile.html',username=username)
    else:
        all_users=Query()
        all_users.send_query(Query.query_accountslist,None)
        liste = all_users.cursor.fetchall()
        if username == liste[0]:
            return render_template('other_profile.html',profile=username,username=Admin.name)
        else: 
            return render_template('not_found.html',url=username,username=Admin.name)
